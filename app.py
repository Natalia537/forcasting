# app.py — Pronóstico Taller (5 métodos + m configurable; tuning & backtesting; sin librerías pesadas)
import io
...
st.title("🔧📈 Pronóstico de Demanda – 5 métodos (m configurable)")

st.markdown("""
Subí un archivo **Excel/CSV** con columnas: **fecha**, **servicio**, **cantidad**, **precio**.

La app normaliza a **mensual**, evalúa 5 métodos con **backtesting** y elige parámetros por **sMAPE**:
**Naive, SeasonalNaive (m), Holt (α,β), Holt-Winters multiplicativo (α,β,γ,m) y Croston (α)**.

> Cambiá el **período estacional (m)** en la barra lateral (p. ej. 4, 6, 12) y verás cómo cambian los métodos estacionales.
""")
...
def tune_method(y: pd.Series, method_name: str, horizon: int, m: int=12) -> Tuple[dict, float]:
    ...
    y = y.astype(float)
    initial = min( max(m*2 if len(y) >= m*2 else max(8, len(y)//2), 8), len(y)-horizon-1 )
    if initial <= 0:
        return ({}, np.inf)
...
st.sidebar.subheader("Parámetros de pronóstico")
horizonte = st.sidebar.selectbox("Horizonte (meses)", [3,6,9,12], index=1)
m = st.sidebar.number_input(
    "Período estacional (m)",
    min_value=2, max_value=24, value=12, step=1,
    help="Ejemplo: 12=anual, 4=cuatrimestral, 6=semestral, etc."
)
...
def export_all_sheets(dfs: Dict[str,pd.DataFrame], metrics: Dict[str,float], fname: str) -> bytes:
    ...
    with pd.ExcelWriter(buf, engine="xlsxwriter") as w:
        ...
        for name, d in dfs.items():
            sheet = name[:31]
            pd.DataFrame({"sMAPE":[metrics.get(name, np.nan)], "m":[m]}).to_excel(w, index=False, sheet_name=sheet, startrow=0)
...
def run_block(title: str, g: pd.DataFrame):
    ...
    if len(y) < max(8, m+2):
        st.warning(f"Serie corta para m={m}. Ideal ≥ {max(8, m*2)} puntos.")
    ...
    fig = px.line(pd.concat([hist, best_df]), x="fecha", y="y", color="serie",
                  title=f"{title} – mejor: {best} (sMAPE={metrics[best]:.2f}, m={m})", markers=True)
    ...
    fig_all = px.line(plot_m, x="fecha", y="yhat", color="modelo",
                      title=f"{title} – comparación de los 5 métodos (m={m})")
    ...
    for tab, (name, d) in zip(tabs, dfs.items()):
        with tab:
            st.write(f"**{name}**  • sMAPE={metrics[name]:.3f} • m={m}")
    ...
    st.download_button(
        ...
        file_name=f"pronosticos_{title.replace(' ','_')}_m{m}_{datetime.now().date()}.xlsx",
